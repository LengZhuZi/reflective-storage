...
| 阶段 | 失败姿态 | 理由 |
| --- | --- | --- |
| 写入（J1–J4） | **fail-open**：照存，标记 `gate='unavailable'` | 丢一条记忆的代价 > 存一条噪声的代价 |
...
1. **每会话只注入一次**（默认）。记忆块必须落在上下文尾部，前面的 system prompt 和历史保持字节级不变，provider 的前缀缓存才能命中。第二次注入会改变前缀，缓存全废。
   ...
   - **实测修正**：严格「一次」在长会话里会变成「后面才出现的话题一条记忆也拿不到」，而无感恰恰要求「聊到 X 的时候 X 的记忆恰好在」。所以默认放宽成**有界多次**：`maxPerSession`（默认 3，设 1 即回到严格一次）、`minTurnsBetween`（默认 3 轮），配置在 `config.json` 的 `inject` 段。
   - **分工不能混**：`maxPerSession` / `minTurnsBetween` 是**机械约束**（前缀缓存的预算、轮次间隔），只有系统自己知道；「这个提问是不是刚才那件事」是**内容判断**，归 J5 —— 不能拿本地二字组规则兼职。所以第二次及以后的注入会真的调 J5，而 J5 那一次调用里同时问两件事（`need_recall` + `new_topic`，多问不涨价），取两者的最小值。首轮不问 J5：§10.4 的第一条规则就是「首轮直接走完整召回」。规则档没有判断力，用 `sameTopic()`（二字组重叠 ≥0.6）当兜底。
...
| J12 遗忘 | 已做（只归档） | `分数<0.05 且 90 天没碰` → archived；**高 importance（≥0.85）永远不归档**（§11）。**不删除** —— 删是不可逆的，本地版交给用户 `/memory forget` |
| J13 复活 | 已做 | 新会话提问命中归档记忆的主题（二字组覆盖 ≥30% 且至少 2 个）就放回 active，每次最多 3 条 |
| J11 合并 | **未做** | 需要引擎级的语义判断。J3 的 `supersedes` 已覆盖最常见的情形（后来的结论取代旧的） |
...
不是每句话都值得调 JEV。`before_agent_start` 里先用 0 成本的本地规则判断：

```
首轮（本会话尚无注入）        → 直接走完整召回
prompt 长度 < 15 字符         → 跳过（"继续"、"好"）
prompt 与当前话题无新实体      → 跳过
上一轮刚注入过且未被压缩        → 跳过
```

规则只负责"**要不要花这次钱**"和机械约束（次数上限、轮次间隔、输入太短），内容判断仍然是判断引擎的活 ——「这次该不该查」是 J5，「这条该不该注」是 J8。
