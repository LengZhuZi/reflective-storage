...
const used = closeFeedbackLoop(o, SESSION, "按这个仓库的约定，一个模块一个提交，我先按 src/jev 和 src/core 拆开。");
...
const termUsed = closeFeedbackLoop(o, SESSION, "日志框架我建议换成 logback 系列，配置集中放一份。");
...
const paraphrased = closeFeedbackLoop(o, SESSION, "上线之前我会先把索引那步补上，再去发版。");
...
const mixed = closeFeedbackLoop(o, SESSION, "影子强度按立面高度算，这个没变；提交那件事照旧。");
...
const attribution = closeFeedbackLoop(o, SESSION, "按约定一个模块一个提交，所以照旧。");
assert.equal(attribution.injected, 1, "归因要落在最近一次真的注入过的那行，不是最后那条搜索");
assert.deepEqual(attribution.cited, [m1.id]);

// ------------------------------------------------------------ 本会话从来没注入过
const none = closeFeedbackLoop(o, "从未注入过的会话", "影子强度按立面高度算。");
...
const gone = closeFeedbackLoop(o, SESSION, "一个模块一个提交，随便说说");
...
const first = closeFeedbackLoop(o, SESSION, "影子强度按立面高度算，就这样。");
assert.deepEqual(first.cited, [m3.id]);
const blank = closeFeedbackLoop(o, SESSION, "   ");
