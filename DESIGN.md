...
1. **每会话只注入一次**（默认）。记忆块必须落在上下文尾部，前面的 system prompt 和历史保持字节级不变，provider 的前缀缓存才能命中。第二次注入会改变前缀，缓存全废。
   ...
   - **实测修正**：严格「一次」在长会话里会变成「后面才出现的话题一条记忆也拿不到」，而无感恰恰要求「聊到 X 的时候 X 的记忆恰好在」。所以默认放宽成**有界多次**：`maxPerSession`（默认 3，设 1 即回到严格一次）、`minTurnsBetween`（默认 3 轮）、`topicOverlapBelow`（默认 0.3，与上次注入时的提问二字组重叠低于此值才算换话题）。三个条件同时满足才再注入，前缀缓存的最坏损失因此有界。配置在 `config.json` 的 `inject` 段。
